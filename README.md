...
In the UML diagram depicted, there are 6 classes. Within the Person, Task, Label, TaskSorter, Importance Sorter, and DateSorter classes, we have their respective private variables/public member functions. We also have the respective getter functions that get inputs from the user which return the respective inputs. Certain classes such as the Task class are dependent on the Label class and the TaskSorter class are dependent on both the ImportanceSorter and DateSorter classes. The TaskSorter, ImportanceSorter, and DateSorter all abide by the 
Interface Segregation Principle (ISP) by having a seperate sorting functions for each of the different filtering methods (i.e. importance and date). Additionally, the Person, Task, and Label classes all abide by the Single Responsibility Principle (SRP) by all focusing on one aspect of the program; for example, the Person class focuses only on the data members and functions that apply only to a Person object, such as their account information; the Task class only focuses on the functionality of a Task object, such as its name, deadline, description, label, and priority; and the Label class only focuses on the attributes of a Task object's label, such as its color, name, and type.
